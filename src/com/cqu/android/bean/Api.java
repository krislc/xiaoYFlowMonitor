...
import com.cqu.android.Activity.IntentList;
import com.cqu.android.Activity.MiscUtil;
...
public final class Api {
		...
		try {
			...
			for (final ApplicationInfo apinfo : installed) {
				...
				if (app == null
				 ...
				 // 过滤系统的应用和电话应用，当然你也可以把它注释掉。----自己添加的
		          if (apinfo.processName.equals("system")
		  				|| apinfo.processName.equals("com.android.phone")) {
		  		     continue;
		       		}
				...
				if (name.length() == 0) {
					...
					//获取中文名-----自己添加的
					name=apinfo.loadLabel(pkgmanager).toString();
					edit.putString(cachekey, name);
					changed = true;
				}
				if (app == null) {
					app = new DroidApp(ctx);
					app.uid = apinfo.uid;
					app.packageName=apinfo.packageName;
					app.names = new String[] { name };
					app.fetchPackageInfo();
					//自己加入的-------
	...
	public static final class DroidApp {
        ...
        public DroidApp(Context ctx) {
        	pm = ctx.getApplicationContext().getPackageManager();
		}

        public void fetchPackageInfo() {
            if (pkginfo == null) pkginfo = MiscUtil.getPackageInfo(pm,packageName);
        }
